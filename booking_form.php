...
<body>
  ...
  <div class="shell">
    ...
    <div class="bnav">
      ...
      <div class="ni on" onclick="goPage('booking_form.php?newbooking=1')"><div class="nb-c"><i class="bi bi-plus-lg"></i></div></div>
  ...
  <script>
    ...
    async function loadServices() {
      try {
        ...
        const svcParam = (params.get('svc') || '').trim().toLowerCase();
        if (svcParam) {
          const idx = services.findIndex(s => String(s.name || '').trim().toLowerCase() === svcParam);
          if (idx >= 0) {
            selectService(idx);
          }
        }
