...
<body>
  ...
  <div class="shell" id="app">
    ...
    <div class="sheet-ol" id="bkDetailOl" onclick="if(event.target===this)closeSheet('bkDetailOl')">
      ...
      <div class="sheet-ol" id="incidentSheetOl" onclick="if(event.target===this)closeSheet('incidentSheetOl')">
        <div class="sheet" style="max-height:92vh;">
          ...
          <div id="incidentSheetBody" style="overflow-y:auto;flex:1;padding:15px 15px 40px;">
            <div class="empty-state">
        ...
        <div class="sheet" style="max-height:92vh;">
          ...
          <div class="sh-hdr">
            <div style="display:flex;align-items:center;gap:8px;">
              ...
              <span id="incidentDetailStatus" class="badge-gray" style="font-size:10px;"></span>
            </div>
            <button class="sh-close" onclick="closeSheet('incidentDetailOl')"><i class="bi bi-x-lg"></i></button>
          </div>
          <div id="incidentDetailBody" style="overflow-y:auto;flex:1;padding:0 0 20px;"></div>
        </div>
      </div>

      <!-- Incident Action Confirmation Modal -->
      <div class="confirm-ol" id="incidentActionConfirmOl" onclick="if(event.target===this)closeIncidentActionConfirm()">
        <div class="confirm-card">
            <div class="confirm-icon" id="incidentActionConfirmIcon" style="background:linear-gradient(135deg,#fee2e2,#fff1f2);color:#dc2626;"><i class="bi bi-shield-exclamation"></i></div>
          <div class="confirm-title" id="incidentActionConfirmTitle">Confirm Action?</div>
          <div class="confirm-sub" id="incidentActionConfirmSub">Are you sure you want to perform this action?</div>
          <div class="confirm-actions">
            <button class="confirm-btn cancel" onclick="closeIncidentActionConfirm()">Cancel</button>
             <button class="confirm-btn ok" id="incidentActionConfirmBtn" onclick="submitIncidentAction()">Yes, Confirm</button>
          </div>
        </div>
      </div>
    ...
    <div class="offer-list-item">
      ...
      // ── Incident Logs (UI Only) ───────────────────────────────────────────────
      const mockIncidents = [
        {
          id: "INC-2026-001",
          booking_id: "BK-8291",
          reporter: "Juana de la Cruz (Homeowner)",
          reported_user: "Robert Santos (Service Provider)",
          type: "No Show",
          date: "2026-06-05 14:30",
          status: "Pending",
          description: "The worker did not show up at the scheduled time of 10:00 AM. I waited for two hours and tried calling several times but got no response.",
          evidence: "No file uploaded",
          notes: "",
          resolution: "",
          booking_info: {
            ...
            date: "2026-06-05",
            price: "1200",
            address: "123 Mabini St., Sto. Tomas, Batangas"
          }
        },
        {
          id: "INC-2026-002",
          booking_id: "BK-7492",
          reporter: "Mark Reyes (Service Provider)",
          reported_user: "Alice Guo (Homeowner)",
          type: "Payment Issue",
          date: "2026-06-04 18:45",
          status: "Under Investigation",
          description: "The homeowner refused to confirm the cash payment of PHP 850 after the cleaning service was successfully completed.",
          evidence: "chat_logs_screenshot.png",
          notes: "Spoke with homeowner Alice Guo, she claims the job was not done properly.",
          resolution: "",
          booking_info: {
            service: "House Cleaner",
            date: "2026-06-04",
        ...
        },
        {
          id: "INC-2026-003",
          booking_id: "BK-6381",
          reporter: "Sarah Johnson (Homeowner)",
          reported_user: "Michael Garcia (Service Provider)",
          type: "Property Damage",
          date: "2026-06-02 11:15",
          status: "Resolved",
          description: "The technician accidentally broke a ceramic vase in the living room while repairing the air conditioning unit.",
          evidence: "broken_vase.jpg",
          notes: "Technician Michael Garcia agreed to compensate for the damaged vase.",
          resolution: "Vase replacement cost of PHP 500 refunded to the homeowner from the provider's earnings. Case closed.",
          booking_info: {
            service: "Appliance Technician",
            date: "2026-06-02",
            price: "1500",
            address: "789 Laurel St., Tanauan, Batangas"
          }
        },
        {
          id: "INC-2026-004",
          booking_id: "BK-9034",
          reporter: "David Lim (Homeowner)",
          reported_user: "John Doe (Service Provider)",
          type: "Harassment",
          date: "2026-05-30 20:00",
          status: "Rejected",
          description: "Provider sent unprofessional text messages after the booking was cancelled.",
          evidence: "unprofessional_texts.png",
          notes: "Text messages reviewed. They were polite confirmation messages, not harassment.",
          resolution: "Report rejected due to lack of evidence of actual harassment.",
          booking_info: {
            service: "Helper",
            date: "2026-05-30",
            price: "2200",
            address: "101 Bonifacio Rd., Batangas City, Batangas"
          }
        }
      ];
      ...
      function renderIncidentLogs() {
        ...
        if (mockIncidents.length === 0) {
          body.innerHTML = '<div class="empty-state"><i class="bi bi-shield"></i><p>No incidents found.</p></div>';
          return;
        }

        body.innerHTML = mockIncidents.map(inc => {
          const statusKey = inc.status.toLowerCase();
          const badgeClass = statusMap[statusKey] || 'badge-gray';
          return `
          <div style="background:var(--bg-card);border-radius:16px;padding:16px;margin-bottom:10px;border:1.5px solid var(--border-col);box-shadow:0 2px 8px rgba(0,0,0,.04);">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
              <div>
                <span style="font-size:12px;font-weight:800;color:var(--txt-primary);">${inc.id}</span>
                <span style="font-size:11px;color:var(--txt-muted);margin-left:6px;">Booking: ${inc.booking_id}</span>
              </div>
              <span class="${badgeClass}" style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;">${inc.status}</span>
            </div>
            <div style="font-size:13px;margin-bottom:10px;">
              <div style="margin-bottom:3px;"><span style="color:var(--txt-muted);font-weight:600;">Reporter:</span> <strong style="color:var(--txt-primary);">${inc.reporter}</strong></div>
              <div><span style="color:var(--txt-muted);font-weight:600;">Reported:</span> <strong style="color:var(--txt-primary);">${inc.reported_user}</strong></div>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <div style="font-size:12px;font-weight:700;color:#d97706;background:#fef3c7;padding:3px 8px;border-radius:6px;">${inc.type}</div>
              <button onclick="openIncidentDetail('${inc.id}')" style="background:transparent;border:1.5px solid var(--border-col);color:var(--txt-muted);font-weight:700;padding:6px 14px;border-radius:10px;font-size:12px;cursor:pointer;transition:all 0.2s;" onmouseover="this.style.borderColor='var(--teal)';this.style.color='var(--teal)'" onmouseout="this.style.borderColor='var(--border-col)';this.style.color='var(--txt-muted)'">View Details</button>
            </div>
          </div>`;
        }).join('');
      }
      ...
      let activeActionSelectEl = null;

      function updateIncidentNotes(id, value) {
        const inc = mockIncidents.find(i => i.id === id);
        if (inc) inc.notes = value;
      }

      function updateIncidentResolution(id, value) {
        const inc = mockIncidents.find(i => i.id === id);
        if (inc) inc.resolution = value;
      }

      function handleIncidentActionChange(selectEl) {
        const value = selectEl.value;
        if (!value) return;

        activeActionSelectEl = selectEl;
        ...
        switch(value) {
          case 'under_investigation':
            actionType = 'Mark as Under Investigation';
            targetStatus = 'Under Investigation';
            message = 'Change this report\'s status to Under Investigation to signal that an admin is reviewing details.';
            isCritical = false;
            break;
          case 'request_info':
            actionType = 'Request Additional Info';
            targetStatus = 'Pending';
            message = 'Request more logs, photos, or details from the parties. Status will remain Pending.';
            isCritical = false;
            break;
          case 'send_warning':
            actionType = 'Send Warning';
            targetStatus = 'Resolved';
            message = 'Issue a formal system warning to the reported user. This will resolve the report.';
            isCritical = false;
            break;
          case 'resolve':
            actionType = 'Resolve Incident';
            targetStatus = 'Resolved';
            message = 'Mark this incident as successfully resolved and close the dispute case.';
            isCritical = false;
            break;
          case 'reject':
            actionType = 'Reject Report';
            targetStatus = 'Rejected';
            message = 'Reject the submitted report as invalid or containing insufficient evidence.';
            isCritical = true;
            break;
          case 'suspend_homeowner':
            actionType = 'Suspend Homeowner Account';
            targetStatus = 'Escalated';
            message = 'Temporarily restrict/suspend the homeowner\'s HomeEase account for violations. This escalates the case status.';
            isCritical = true;
            break;
          case 'suspend_provider':
            actionType = 'Suspend Provider Account';
            targetStatus = 'Escalated';
            message = 'Temporarily suspend the service provider\'s account from accepting jobs. This escalates the case status.';
            isCritical = true;
            break;
        }

        if (actionType) {
          confirmIncidentAction(actionType, targetStatus, message, isCritical);
        }
      }

      function confirmIncidentAction(actionType, targetStatus, message, isCritical = false) {
        currentAction = { actionType, targetStatus, message };
        
        const overlay = document.getElementById('incidentActionConfirmOl');
        const title = document.getElementById('incidentActionConfirmTitle');
        const sub = document.getElementById('incidentActionConfirmSub');
        const okBtn = document.getElementById('incidentActionConfirmBtn');
        const icon = document.getElementById('incidentActionConfirmIcon');

        title.textContent = `Confirm ${actionType}?`;
        sub.textContent = message;
        okBtn.textContent = actionType;
        
        if (isCritical) {
          icon.style.background = 'linear-gradient(135deg,#fee2e2,#fff1f2)';
          icon.style.color = '#dc2626';
          okBtn.style.background = 'linear-gradient(135deg, #EF4444, #F87171)';
          okBtn.style.color = '#fff';
          okBtn.style.boxShadow = '0 8px 16px rgba(239, 68, 68, .28)';
        } else {
          icon.style.background = 'linear-gradient(135deg,#dbeafe,#eff6ff)';
          icon.style.color = '#2563eb';
          okBtn.style.background = 'linear-gradient(135deg, #059669, #10B981)';
          okBtn.style.color = '#fff';
          okBtn.style.boxShadow = '0 8px 16px rgba(5, 150, 105, .28)';
        }

        overlay.classList.add('on');
      }

      function closeIncidentActionConfirm() {
        const overlay = document.getElementById('incidentActionConfirmOl');
        if (overlay) overlay.classList.remove('on');
        
        // Revert select dropdown to empty option if canceled or closed
        if (activeActionSelectEl) {
          activeActionSelectEl.value = '';
          activeActionSelectEl = null;
        }
        currentAction = null;
      }

      function submitIncidentAction() {
        if (!currentAction || !currentIncidentId) return;

        const inc = mockIncidents.find(i => i.id === currentIncidentId);
        if (inc) {
          inc.status = currentAction.targetStatus;
          
          // Apply changes to notes & resolution inputs in object
          const notesInput = document.getElementById('incNotesInput');
          const resInput = document.getElementById('incResolutionInput');
          if (notesInput) inc.notes = notesInput.value;
          if (resInput) inc.resolution = resInput.value;

          toast(`Incident status updated to: ${inc.status}`, 's');
        }

        closeIncidentActionConfirm();
        renderIncidentLogs();
      ...
      function openIncidentDetail(id) {
        ...
        const statusSpan = document.getElementById('incidentDetailStatus');
        ...
        <div style="padding:16px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;background:var(--bg-screen);border-radius:12px;padding:12px 14px;margin-bottom:16px;border:1.5px solid var(--border-col);">
            <span style="font-size:12px;font-weight:800;color:var(--txt-muted);text-transform:uppercase;">Current Status</span>
            <span class="${badgeClass}" style="font-size:11px;font-weight:800;padding:4px 10px;border-radius:20px;">${incident.status}</span>
          </div>

          <!-- Incident Basic Info -->
          <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Incident Information</div>
          <div style="background:var(--bg-screen);border-radius:14px;padding:14px;margin-bottom:16px;border:1.5px solid var(--border-col);">
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Incident ID</span>
              <span style="font-size:13px;font-weight:800;color:var(--txt-primary);">${incident.id}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Date Submitted</span>
              <span style="font-size:13px;font-weight:700;color:var(--txt-primary);">${incident.date}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Incident Type</span>
              <span style="font-size:13px;font-weight:800;color:#d97706;">${incident.type}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Reporter Name</span>
              <span style="font-size:13px;font-weight:700;color:var(--txt-primary);">${incident.reporter}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Reported User</span>
              <span style="font-size:13px;font-weight:700;color:var(--txt-primary);">${incident.reported_user}</span>
            </div>
          </div>

          <!-- Booking Info Section -->
          <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Booking Information</div>
          <div style="background:var(--bg-screen);border-radius:14px;padding:14px;margin-bottom:16px;border:1.5px solid var(--border-col);">
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Booking ID</span>
              <span style="font-size:13px;font-weight:800;color:var(--txt-primary);">${incident.booking_id}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Service Type</span>
              <span style="font-size:13px;font-weight:800;color:var(--txt-primary);">${incident.booking_info.service}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Booking Date</span>
              <span style="font-size:13px;font-weight:700;color:var(--txt-primary);">${incident.booking_info.date}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid var(--border-col);padding-bottom:8px;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Total Price</span>
              <span style="font-size:13px;font-weight:800;color:var(--teal);">₱${Number(incident.booking_info.price).toLocaleString()}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
              <span style="font-size:11px;font-weight:700;color:var(--txt-muted);text-transform:uppercase;">Location Address</span>
              <span style="font-size:13px;font-weight:700;color:var(--txt-primary);max-width:65%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${incident.booking_info.address}">${incident.booking_info.address}</span>
            </div>
          </div>

          <!-- Description -->
          <div style="margin-bottom:16px;">
            <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Incident Description</div>
            <div style="font-size:13px;color:var(--txt-primary);line-height:1.6;background:var(--bg-card);border:1.5px solid var(--border-col);border-radius:12px;padding:12px 14px;white-space:pre-wrap;">${incident.description}</div>
          </div>

          <!-- Uploaded Evidence -->
          <div style="margin-bottom:16px;">
            <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Evidence Uploaded</div>
            <div style="font-size:13px;color:var(--txt-primary);background:var(--bg-card);border:1.5px solid var(--border-col);border-radius:12px;padding:12px 14px;">
              ${incident.evidence === 'No file uploaded' 
                ? `<div style="color:var(--txt-muted);font-style:italic;"><i class="bi bi-file-earmark-slash" style="margin-right:6px;"></i>No evidence files uploaded</div>`
                : `<div style="display:flex;align-items:center;justify-content:space-between;">
                     <span><i class="bi bi-image" style="color:var(--teal);margin-right:6px;"></i>${incident.evidence}</span>
                     <button onclick="toast('Opening mock evidence preview...')" style="background:transparent;border:none;color:var(--teal);font-weight:700;font-size:12px;cursor:pointer;">View File</button>
                   </div>`
              }
            </div>
          </div>
          ...
          <!-- Investigation Notes -->
          <div style="margin-bottom:16px;">
            <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Investigation Notes &amp; Findings</div>
            <textarea id="incNotesInput" oninput="updateIncidentNotes('${incident.id}', this.value)" placeholder="Enter investigation notes, findings, and logs here..." style="width:100%;min-height:90px;padding:10px 12px;border:1.5px solid var(--border-col);border-radius:12px;font-family:'Nunito',sans-serif;font-size:13px;color:var(--txt-primary);background:var(--bg-card);resize:vertical;outline:none;" onfocus="this.style.borderColor='#f5a623'" onblur="this.style.borderColor='var(--border-col)'">${incident.notes || ''}</textarea>
          </div>

          <!-- Resolution Summary -->
          <div style="margin-bottom:20px;">
            <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.3px;">Resolution Summary</div>
            <textarea id="incResolutionInput" oninput="updateIncidentResolution('${incident.id}', this.value)" placeholder="Enter final decision and resolution summary before closing..." style="width:100%;min-height:90px;padding:10px 12px;border:1.5px solid var(--border-col);border-radius:12px;font-family:'Nunito',sans-serif;font-size:13px;color:var(--txt-primary);background:var(--bg-card);resize:vertical;outline:none;" onfocus="this.style.borderColor='#f5a623'" onblur="this.style.borderColor='var(--border-col)'">${incident.resolution || ''}</textarea>
          </div>

          <!-- Admin Actions Section -->
          <div style="margin-bottom:24px;border-top:1.5px dashed var(--border-col);padding-top:20px;">
            <div style="font-size:12px;font-weight:800;color:var(--txt-muted);margin-bottom:8px;text-transform:uppercase;letter-spacing:0.3px;">Take Admin Action</div>
            <select id="incidentActionSelect" class="admin-select" onchange="handleIncidentActionChange(this)">
              <option value="" disabled selected>-- Select Action to Apply --</option>
              <optgroup label="Investigation & Info">
                <option value="under_investigation">Mark as Under Investigation</option>
                <option value="request_info">Request Additional Info</option>
              </optgroup>
              <optgroup label="Resolution & Warnings">
                <option value="send_warning">Send Warning</option>
                <option value="resolve">Resolve Incident</option>
              </optgroup>
              <optgroup label="Critical Actions">
                <option value="reject">Reject Report</option>
                <option value="suspend_homeowner">Suspend Homeowner</option>
                <option value="suspend_provider">Suspend Provider</option>
              </optgroup>
            </select>
          </div>

          <div class="modal-btns">
            <button class="btn-outline" onclick="closeSheet('incidentDetailOl')" style="width:100%;border-radius:50px;padding:13px;font-weight:800;font-family:'Poppins',sans-serif;font-size:14px;border:2.5px solid var(--border-col);background:transparent;color:var(--txt-muted);cursor:pointer;">Go Back to List</button>
          </div>
        </div>`;
