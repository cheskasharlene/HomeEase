...
// Admin is a standalone site — use its own local db.php
require_once __DIR__ . '/db.php';

$action = $_GET['action'] ?? $_POST['action'] ?? '';

if ($action === 'ping') {
    echo json_encode(['success' => true, 'message' => 'admin_api reachable', 'time' => date('Y-m-d H:i:s')]);
    exit;
}
...
if ($action === 'get_bookings') {
    ...
    $sql = $hasTech
        ? "SELECT b.*, u.name AS customer_name, t.name AS technician_name FROM bookings b LEFT JOIN users u ON b.user_id=u.id LEFT JOIN technicians t ON b.technician_id=t.id"
        : "SELECT b.*, u.name AS customer_name, NULL AS technician_name FROM bookings b LEFT JOIN users u ON b.user_id=u.id";

    if ($status !== 'all') $sql .= " WHERE b.status=" . $conn->real_escape_string("'$status'");
    ...
    $stmt->execute();
    echo json_encode(['success'=>true, 'bookings'=>$stmt->get_result()->fetch_all(MYSQLI_ASSOC)]);
    exit;
}
...
if ($action === 'get_technicians') {
    ...
    if (!$result) { echo json_encode(['success'=>false,'message'=>$conn->error,'technicians'=>[]]); exit; }
...
if ($action === 'save_technician') {
    ...
    if (!$name||!$spec) { echo json_encode(['success'=>false,'message'=>'Name and specialty required']); exit; }
    if ($id) {
...
if ($action === 'get_services') {
    ...
    if (!$result) { echo json_encode(['success'=>false,'message'=>'Services table not found','services'=>[]]); exit; }
...
if ($action === 'get_offers') {
    ...
    if (!$result) { echo json_encode(['success'=>false,'message'=>$conn->error,'offers'=>[]]); exit; }
...
if ($action === 'save_offer') {
    ...
    $active  = intval($_POST['active']??1);
    if (!$title||!$code) { echo json_encode(['success'=>false,'message'=>'Title and code required']); exit; }
    if ($id) {
        $stmt = $conn->prepare("UPDATE special_offers SET title=?,code=?,description=?,discount_type=?,discount_value=?,min_booking_price=?,max_uses=?,expires_at=?,active=? WHERE id=?");
        $stmt->bind_param("ssssddisii",$title,$code,$desc,$dtype,$dval,$minprice,$maxuses,$expires,$active,$id);
    } else {
        $stmt = $conn->prepare("INSERT INTO special_offers (title,code,description,discount_type,discount_value,min_booking_price,max_uses,expires_at,active) VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("ssssddisi",$title,$code,$desc,$dtype,$dval,$minprice,$maxuses,$expires,$active);
    }
    $ok = $stmt->execute();
    if (!$ok) { echo json_encode(['success'=>false,'message'=>$conn->error]); exit; }
