<?php
session_start();
if (empty($_SESSION['provider_id'])) { header('Location: provider_index.php'); exit; }
...
<head>
  ...
  <style>
    ...
    .wfp-status-banner.accepted {
        background: linear-gradient(135deg, #1A1A2E, #2D2D4E); /* Dark theme for provider en route */
    }
    .mark-done-btn {
        width: 100%;
        height: 46px;
...
<body>
  <div class="wfp-shell">
    ...
    <div class="wfp-style-overlay" id="styleOverlay" style="position:absolute;inset:0;z-index:700;background:rgba(0,0,0,.45);backdrop-filter:blur(4px);display:flex;align-items:flex-end;opacity:0;pointer-events:none;transition:opacity .28s" onclick="closeStylePicker(event)">
      ...
      <div class="wfp-sheet-body" id="sheetBody">
        ...
        <div class="wfp-provider-card" id="clientCard">
          ...
          <div class="wfp-prov-info">
            ...
            <div class="wfp-prov-meta" id="clientAddress" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">–</div>
            <div class="wfp-prov-rating" id="clientNotes" style="color:#7A7064;font-weight:600;">–</div>
          </div>
  ...
  <div id="chatOverlay" style="position:absolute;inset:0;z-index:800;background:rgba(0,0,0,.45);backdrop-filter:blur(3px);display:flex;align-items:flex-end;justify-content:center;opacity:0;pointer-events:none;transition:opacity .25s" onclick="closeChat(event)">
    <div id="chatDrawer" style="width:100%;max-width:480px;background:#fff;border-radius:24px 24px 0 0;padding-bottom:32px;display:flex;flex-direction:column;max-height:72dvh;transform:translateY(100%);transition:transform .32s cubic-bezier(.32,.72,0,1)" onclick="event.stopPropagation()">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px 10px;border-bottom:1.5px solid #F0EAE0">
        <div style="font-size:15px;font-weight:800;color:#1A1A2E;font-family:'Poppins',sans-serif">💬 Chat with Client</div>
  ...
  <script>
    ...
    const STO_TOMAS = {
      lat: 14.1053, lng: 121.1390,  // Service hub center
      bounds: L.latLngBounds(L.latLng(13.9800, 120.9800), L.latLng(14.2500, 121.3200))
    };
    ...
    let custLat = 14.1053, custLng = 121.1390;  // Sto. Tomas, Batangas
    let provLat = 14.1053, provLng = 121.1390;
    ...
    function initMap() {
      ...
      }).setView([STO_TOMAS.lat, STO_TOMAS.lng], 15);
      ...
      if (navigator.geolocation) {
        ...
        function _onFix(pos) {
          ...
          if (!isInStoTomas(rawLat, rawLng)) {
            showGpsStatus('error', '📍 GPS outside Sto. Tomas service area — using map default.');
            return;
          }

          const threshold = _hasFirst ? _GPS.ACCEPT_STEADY : _GPS.ACCEPT_FIRST;
          if (acc > threshold) {
            showGpsStatus('loading', `⏳ Improving accuracy… ±${Math.round(acc)}m`);
            if (!_hasFirst) {
              const s = _smooth(rawLat, rawLng);
              provLat = s.lat; provLng = s.lng;
              updateMarkers(); _hasFirst = true;
            }
            return;
          }
          ...
          if (!moved && !betterAcc && _hasFirst) return;

          _bestAcc = Math.min(_bestAcc, acc);
          _hasFirst = true;
          provLat = s.lat;
          provLng = s.lng;
          updateMarkers();
          ...
          const label = acc < 5  ? `✅ ${Math.round(acc)}m — Excellent`
                      ...
                      : acc < 50 ? `📍 ±${Math.round(acc)}m — Good`
                      :            `⚠️ ±${Math.round(acc)}m — Low`;
          showGpsStatus('success', label);
          setTimeout(() => hideGpsStatus(), 5000);
        }
        ...
        const _opts = { enableHighAccuracy: true, maximumAge: 0, timeout: 20000 };
        showGpsStatus('loading', '📡 Acquiring GPS signal…');

        // Fast initial fix, then continuous watch
        navigator.geolocation.getCurrentPosition(_onFix, _onErr, { ..._opts, timeout: 8000 });
    ...
    async function geocode(address) {
      try {
        const q = encodeURIComponent(address + ', Sto. Tomas, Batangas, Philippines');
        const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${q}&format=json&limit=1&viewbox=121.085,14.055,121.195,14.155&bounded=1`, { headers:{'Accept-Language':'en'} });
    ...
    async function loadBooking() {
      try {
        ...
        if (provLat === 14.1053) {
            provLat = custLat - 0.012;
            provLng = custLng - 0.008;
        }
        updateMarkers();
